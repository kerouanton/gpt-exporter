...
- Audit results are written to `reports\asset-reference-audit-v2.7.json.xz`. Discrepancies are warnings by default; `audit_asset_references.py --strict` returns exit code 2 when discrepancies exist.
...
- With the STL attachment plus the two generated images present locally, the v2.7 audit found 3 unique local asset IDs, 3 rendered asset IDs, zero unreferenced local assets, and zero output references missing locally.

### Preservation policy

- Browser collection, asset caching, cumulative `.json.xz` storage, and broad over-collection are unchanged from v2.6.
- The new audit is diagnostic only: it never deletes, moves, filters, or rewrites an asset.
- Existing DOCX files are not silently rebuilt on installation. Run `py archive_chats.py --convert-only` once after upgrading to v2.7 to rebuild historical DOCX files with the new generated-image handling and obtain a meaningful whole-archive audit.

## v2.6 — Frozen reference release — 2026-08-17
...
- `collect_chatgpt_archive.js` is unchanged from the v2.4 collector baseline.
- Broad asset collection remains enabled; v2.6 does **not** narrow candidate detection.
...
- Atlantis Word Processor was tested manually with four relative-link forms: forms using forward slashes failed, while Windows forms `assets\...` and `.\assets\...` succeeded; v2.6 deliberately uses the explicit `.\assets\...` form.
...
- sandbox basenames with multiple local matches, where v2.6 refuses to guess;
...
## v2.5 — DOCX asset references — intermediate release — 2026-08-17
...
These issues were corrected in v2.6. v2.5 is not the frozen reference release.

## Experimental `structured-assets-v2.5` — rejected / not part of the release lineage
...
**Decision retained in v2.6:** keep the v2.4 broad collector unchanged; tolerate stale fetch errors and optimize only with caches, never by narrowing preservation coverage.

## v2.4 — Robust DOCX image embedding — 2026-08-17
...
## v2.3 — Legacy asset recovery from the physical archive — 2026-08-17
...
v2 established the architecture on which v2.3-v2.6 are based.
