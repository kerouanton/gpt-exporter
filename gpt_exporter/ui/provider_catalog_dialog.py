"""Read-only provider catalog comparison UI."""
...
from gpt_exporter.version import APP_NAME, __version__
...
class ProviderCatalogDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        *,
        ...
        ttk.Label(
            outer,
            text=f"Generated: {catalog.generated_at} — MSNE {__version__} — read-only preview",
        ...
        self.tree.column("state", width=140, minwidth=110)
        ...
        self.tree.column("distribution", width=250, minwidth=170)
        ...
        for index, entry in enumerate(catalog.entries, start=1):
            ...
            self.tree.insert(
                ...
                values=(
                    self._catalog_state(entry, record),
        ...
        if children:
            ...
            self._set_details("This catalog contains no provider entries.")
    ...
    def _catalog_state(
        ...
        if not entry.compatible_provider_api or not entry.supports_msne(__version__):
            return "Incompatible"
    ...
    def _selection_changed(self, _event: tk.Event | None = None) -> None:
        ...
        lines.append(
            "This view is read-only. Catalog-originated download/install is not enabled yet."
        )
        self._set_details("\n".join(lines))
