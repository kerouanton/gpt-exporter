...
from gpt_exporter.core.provider_discovery import PROVIDER_API_VERSION, PROVIDER_ENTRY_POINT_GROUP
...
def _fresh_provider_modules(root: Path, module_names: tuple[str, ...]):
    """Temporarily load provider modules from ``root`` without poisoning the app process."""
...
def install_provider_with_validation(
    ...
    try:
        ...
        try:
            provider_ids = validate_managed_provider_runtime(destination, installed)
